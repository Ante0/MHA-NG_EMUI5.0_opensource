...
static int do_resize(struct f2fs_sb_info *sbi)
{
	...
	if (config.target_sectors ==
			(get_sb(block_count) << get_sb(log_sectors_per_block))) { /*lint !e574*/
		ASSERT_MSG("Nothing to resize; Just support enlarge");
		return -1;
	}

	MSG(0, "\t Begin to resize from sector: 0x%"PRIx64" to sector: 0x%"PRIx64"!\n",
		get_sb(block_count) << get_sb(log_sectors_per_block), config.target_sectors);
...
out:
	...
	if (is_first_fsck >= 0 && is_first_fsck < 2)
		dmd_report(client, DMD_F2FSCK_ERR, LOG_TYP_FSCK);

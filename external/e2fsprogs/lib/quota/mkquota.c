 ...
 * inode number.
 */
int quota_file_exists(ext2_filsys fs, int qtype, _UNUSED int fmt)
{
	char qf_name[256];
	errcode_t ret;
	ext2_ino_t ino;
 ...
 */
void quota_data_add(quota_ctx_t qctx, struct ext2_inode *inode, _UNUSED ext2_ino_t ino,
		    qsize_t space)
{
	struct dquot	*dq;
	dict_t		*dict;
 ...
 */
void quota_data_sub(quota_ctx_t qctx, struct ext2_inode *inode, _UNUSED ext2_ino_t ino,
		    qsize_t space)
{
	struct dquot	*dq;
	dict_t		*dict;
...
void quota_data_inodes(quota_ctx_t qctx, struct ext2_inode *inode,
		       _UNUSED ext2_ino_t ino, int adjust)
{
	struct dquot	*dq;
	dict_t		*dict;
	int		i;

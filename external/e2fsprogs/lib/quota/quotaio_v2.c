...
static int v2_check_file(struct quota_handle *h, int type, int fmt)
{
	...
	if (ext2fs_le32_to_cpu(dqh.dqh_magic) != (__u32)file_magics[type]) {
		if (ext2fs_be32_to_cpu(dqh.dqh_magic) == (__u32)file_magics[type])
 ...
 */
static int v2_report(_UNUSED struct quota_handle *h, _UNUSED int verbose)
{

...
static int probe_zfs(_UNUSED struct blkid_probe *probe, _UNUSED struct blkid_magic *id,
		     _UNUSED unsigned char *buf)
{
#if 0
	char *vdev_label;
    ...
    unsigned char *buf)
{
    ...
    if (label) {
        char utf8_label[128];
        unicode_16le_to_utf8((unsigned char *)utf8_label, sizeof(utf8_label), label->name, label->length * 2);

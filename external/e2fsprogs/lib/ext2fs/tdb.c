...
static int _tdb_lock(struct tdb_context *tdb, int list, int ltype, int op)
{
	...
	if (tdb->global_lock.count &&
	    ((unsigned int)ltype == tdb->global_lock.ltype || ltype == F_RDLCK)) {
		return 0;
	}
...
static int _tdb_lockall(struct tdb_context *tdb, int ltype, int op)
{
	...
	if (tdb->global_lock.count && tdb->global_lock.ltype == (unsigned int)ltype) {
...
static int _tdb_unlockall(struct tdb_context *tdb, int ltype)
{
	...
	if (tdb->global_lock.ltype != (unsigned int)ltype || tdb->global_lock.count == 0) {
		return TDB_ERRCODE(TDB_ERR_LOCK, -1);
	}
...
*/
static int transaction_oob(struct tdb_context *tdb, tdb_off_t len, _UNUSED int probe)
{
	if (len <= tdb->map_size) {
		return 0;
	}
...
*/
static int transaction_brlock(_UNUSED struct tdb_context *tdb, _UNUSED tdb_off_t offset,
			      _UNUSED int rw_type, _UNUSED int lck_type, _UNUSED int probe, _UNUSED size_t len)
{
	return 0;
}
...
/*
  sync to disk
*/
static int transaction_sync(struct tdb_context *tdb, _UNUSED tdb_off_t offset, _UNUSED tdb_len_t length)
{
	if (fsync(tdb->fd) != 0) {
		tdb->ecode = TDB_ERR_IO;
...
static int tdb_key_compare(TDB_DATA key, TDB_DATA data, _UNUSED void *private_data)
{
...
static void null_log_fn(_UNUSED struct tdb_context *tdb, _UNUSED enum tdb_debug_level level, _UNUSED const char *fmt, ...)
{
}
